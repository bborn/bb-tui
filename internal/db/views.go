...
// DefaultSavedViews replaces the starter views. bb-tui sets its own, because
// TaskYou's are written in TaskYou's vocabulary: its board has an In Progress
// column and a Blocked one, and bb's has Running and Waiting on you.
func DefaultSavedViews(views []SavedView) {
	if len(views) > 0 {
		defaultSavedViews = views
	}
}
