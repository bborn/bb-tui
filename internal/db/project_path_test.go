...
func TestExpandHomePath(t *testing.T) {
	...
	cases := []struct {
		...
		{"~/Projects/rails/acmestore", filepath.Join(home, "Projects/rails/acmestore")},
		{"~", home},
...
// "cannot change to '~/Projects/rails/acmestore'" — which is how a pipeline lost
...
func TestProjectPathIsExpandedOnWriteAndRead(t *testing.T) {
	...
	want := filepath.Join(home, "Projects/rails/acmestore")
	p := &Project{Name: "acmestore", Path: "~/Projects/rails/acmestore"}
	...
	got, err := database.GetProjectByName("acmestore")
	if err != nil || got == nil {
	...
	for _, lp := range list {
		if lp.Name == "acmestore" && lp.Path != want {
...
func TestLegacyTildeRowIsExpandedOnRead(t *testing.T) {
	...
	if _, err := database.Exec(`INSERT INTO projects (name, path) VALUES (?, ?)`,
		"legacy", "~/Projects/rails/globexkit"); err != nil {
		t.Fatal(err)
	}
	...
	if want := filepath.Join(home, "Projects/rails/globexkit"); got.Path != want {
