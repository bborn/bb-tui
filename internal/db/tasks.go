...
func (db *DB) ListTasks(opts ListTasksOptions) ([]*Task, error) {
	if db.backend != nil {
		return db.backend.ListTasks(opts)
	}
	query := `
		SELECT id, title, body, status, type, project, COALESCE(executor, 'claude'),
		       worktree_path, branch_name, port, claude_session_id,
...
func (db *DB) SearchTasks(query string, limit int) ([]*Task, error) {
	if db.backend != nil {
		return db.backend.SearchTasks(query, limit)
	}
	if limit <= 0 {
		limit = 50
	}
...
func (db *DB) CountTasksByStatus(status string) (int, error) {
	if db.backend != nil {
		return db.backend.CountTasksByStatus(status)
	}
	var count int
...
func (db *DB) UpdateTaskLastAccessedAt(taskID int64) error {
	if db.backend != nil {
		return db.backend.MarkOpened(taskID)
	}
	_, err := db.Exec(`
...
func (db *DB) GetTaskLogs(taskID int64, limit int) ([]*TaskLog, error) {
	if db.backend != nil {
		return db.backend.GetTaskLogs(taskID, limit)
	}
...
func (db *DB) GetLatestLogPerTask(taskIDs []int64) (map[int64]*TaskLog, error) {
	if db.backend != nil {
		return db.backend.GetLatestLogPerTask(taskIDs)
	}
	if len(taskIDs) == 0 {
		return nil, nil
	}
...
func (db *DB) GetTaskLogCount(taskID int64) (int, error) {
	if db.backend != nil {
		return db.backend.GetTaskLogCount(taskID)
	}
	var count int
...
func (db *DB) CountTasksByProject(projectName string) (int, error) {
	if db.backend != nil {
		return db.backend.CountTasksByProject(projectName)
	}
	var count int
...
func (db *DB) ListProjects() ([]*Project, error) {
	if db.backend != nil {
		return db.backend.ListProjects()
	}
	rows, err := db.Query(`
