...
func TestSavedViewCRUD(t *testing.T) {
	...
	v, err := database.SaveView("Acmestore", "[acmestore] status:open")
	if err != nil {
		t.Fatalf("save view: %v", err)
	}
	if v.Name != "Acmestore" || v.Query != "[acmestore] status:open" {
		t.Fatalf("stored view = %+v", v)
	}

	got, err := database.GetSavedView("acmestore")
	if err != nil || got == nil {
		t.Fatalf("lookup must be case-insensitive: %v %+v", err, got)
	}

	// Saving over a name replaces the query rather than creating a twin.
	if _, err := database.SaveView("acmestore", "[acmestore] is:pinned"); err != nil {
		t.Fatalf("resave: %v", err)
	}
	got, _ = database.GetSavedView("Acmestore")
	if got.Query != "[acmestore] is:pinned" {
	...
	if err := database.DeleteSavedView("ACMESTORE"); err != nil {
