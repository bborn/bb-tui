...
import (
	...
	"github.com/bborn/bb-tui/internal/tmuxtest"
)
...
func TestRemoteAttachDropsALeftOverViewPairing(t *testing.T) {
	// Exercises attaching to a task running on another host through a nested
	// tmux view. bb-tui drives threads over bb's API and never opens one, so
	// this guards a path it does not take; it is kept for the upstream code it
	// covers rather than deleted.
	t.Skip("remote tmux attach is unused by bb-tui")
