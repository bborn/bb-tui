...
func TestFilterAutocompleteSelect(t *testing.T) {
	m := &FilterAutocompleteModel{
		projects: []*db.Project{{Name: "acmestore"}, {Name: "workflow"}},
		maxShow:  5,
	}

	if name := m.Select(); name != "acmestore" {
		t.Errorf("Select() = %q, want acmestore", name)
	}
...
func TestFilterAutocompleteView(t *testing.T) {
	m := &FilterAutocompleteModel{
		projects: []*db.Project{{Name: "acmestore"}},
		maxShow:  5,
	}
	...
	if !contains(view, "[acmestore]") {
		t.Error("view should contain [acmestore]")
	}
}
...
func TestFilterAutocompleteModeFollowsTheChipBeingTyped(t *testing.T) {
	...
	if err := database.CreateProject(&db.Project{Name: "acmestore", Path: "/tmp/acmestore"}); err != nil {
