...
type DebugDetail struct {
	...
	// Scroll state, so thread navigation can be checked without a terminal.
	ScrollOffset   int  `json:"scroll_offset"`
	ScrollTotal    int  `json:"scroll_total"`
	AtBottom       bool `json:"at_bottom"`
	StickyBottom   bool `json:"sticky_bottom"`
	MessageAnchors int  `json:"message_anchors"`
	ComposerFocus  bool `json:"composer_focused"`
...
func (m *AppModel) GenerateDebugState() DebugState {
	...
	if m.currentView == ViewDetail && m.detailView != nil && m.selectedTask != nil {
		s.Detail = &DebugDetail{
			...
			ScrollOffset:   m.detailView.viewport.YOffset,
			ScrollTotal:    m.detailView.viewport.TotalLineCount(),
			AtBottom:       m.detailView.viewport.AtBottom(),
			StickyBottom:   m.detailView.stickyBottom,
			MessageAnchors: len(m.detailView.messageAnchors),
			ComposerFocus:  m.detailView.ComposerFocused(),
		}
	}
...
// SetMouseCaptured records whether the program started with mouse reporting on,
// so the toggle knows which way to go first.
func (m *AppModel) SetMouseCaptured(captured bool) { m.mouseCaptured = captured }

// DebugReloadDetail re-reads the open thread. The harness needs it because a
// conversation is fetched in the background: the first paint after opening a
// thread legitimately has no logs, so a check that scrolls would be scrolling
// nothing.
func (m *AppModel) DebugReloadDetail() tea.Cmd {
	if m.currentView != ViewDetail || m.selectedTask == nil {
		return nil
	}
	return m.loadTask(m.selectedTask.ID)
}

// DebugDetailLogCount reports how much of the conversation has arrived.
func (m *AppModel) DebugDetailLogCount() int {
	if m.detailView == nil {
		return 0
	}
	return len(m.detailView.logs)
}
