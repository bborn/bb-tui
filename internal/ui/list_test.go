...
func listSampleTasks() []*db.Task {
	return []*db.Task{
		...
		{ID: 2, Title: "Running now", Status: db.StatusProcessing, Project: "acmestore"},
		{ID: 3, Title: "Waiting on me", Status: db.StatusBlocked, Project: "acmestore"},
...
func TestListModeRendersOneLinePerTask(t *testing.T) {
	...
	// Project labels are abbreviated the same way the kanban card does it: a
	// long single word is truncated rather than mapped to a fixed nickname.
	if !strings.Contains(out, "[acmest") {
