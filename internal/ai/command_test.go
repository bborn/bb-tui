...
func TestParseResponse_WithProject(t *testing.T) {
	...
	response := `{"type":"create_task","title":"Add dark mode","project":"acmestore","message":"Creating task in acmestore: Add dark mode"}`
	cmd, err := s.parseResponse(response, "new task in acmestore: add dark mode")
	...
	if cmd.Project != "acmestore" {
		t.Errorf("Project = %q, want %q", cmd.Project, "acmestore")
	}
}
