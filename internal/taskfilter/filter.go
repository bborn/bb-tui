...
var statusAliases = map[string][]string{
	...
	// bb's own words for the same states, so a filter can be written in the
	// vocabulary the board actually shows.
	"waiting":     {db.StatusBlocked},
	"attention":   {db.StatusBlocked},
	"idle":        {db.StatusDone},
	"done":        {db.StatusDone},
...
func StatusTokens() []string {
	return []string{
		"backlog", "queued", "processing", "in-progress", "running", "blocked",
		"waiting", "done", "idle", "archived", "open",
	}
}
