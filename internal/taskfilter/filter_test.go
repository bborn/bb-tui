...
func TestParseProjects(t *testing.T) {
	q := Parse("[acmestore] [GlobexKit] bump deps")
	if !reflect.DeepEqual(q.Projects, []string{"acmestore", "globexkit"}) {
	...
	mixed := Parse("is:task [acmestore] bump")
...
func TestMatch(t *testing.T) {
	blocked := &db.Task{ID: 1, Status: db.StatusBlocked, Project: "acmestore"}
...
func TestMatchProjectAndKeyword(t *testing.T) {
	task := &db.Task{ID: 77, Title: "Fix the retry banner", Project: "AcmeStore", Body: "seen in staging"}

	if !Parse("[acmestore]").MatchProject(task) {
		t.Error("project match should be case-insensitive")
	}
	if Parse("[globexkit]").MatchProject(task) {
...
func TestResolveProjects(t *testing.T) {
	...
	if !reflect.DeepEqual(q.Projects, []string{"acmestore", "unknown"}) {
	...
	if q.MatchProject(&db.Task{Project: "globexkit"}) {
...
func TestParseHosts(t *testing.T) {
	cases := []struct {
		...
		{"[acmestore] @mona bump", []string{"mona"}, "[acmestore] bump"},
